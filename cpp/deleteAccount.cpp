...
int main(int argc, char* argv[]) {
    ...
    string deleteAccountName = argv[2];
    ...
    int accountIndex;
    for (int i = 0; i < bankAccounts.size(); i++) {
        if (bankAccounts[i].accountName == deleteAccountName) {
            foundAccount = true;
            accountIndex = i;
        }
    }
    ...
    if (foundAccount) {
        bankAccounts.erase(bankAccounts.begin() + accountIndex);
        writeAccountToFile(filename, username, bankAccounts);

        cout << "True;";
    }
    else {
        cout << "False;cannot delete account: account not found";
    }
